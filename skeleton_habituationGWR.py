...
x = pls*6 + [0]*10 + pls*6 # rest period added

v = stv # Set connection weight to start weight value

forgetflag=True # if true, allows model to simulate process
                # of forgetting the new association 
                # if pulse is not felt for a while, forget 
                # the association
############################### 
...
for t in range(nTs):
    ...
    if t>3 and 0==sum(x[t-4:t]) and v<stv and forgetflag:
        v+=(stv-v)*.05 # forget the association if it doesn't happen for a while
